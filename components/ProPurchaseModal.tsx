...
interface ProPurchaseModalProps {
  ...
  initialPlan?: 'monthly' | 'lifetime';
}

export const ProPurchaseModal: React.FC<ProPurchaseModalProps> = ({
  onClose, onPurchase, onRestore, onRetry, purchasing,
  offeringsLoading, offeringsError, offeringsErrorMsg, monthlyPlan, lifetimePlan, initialPlan,
}) => {
  const defaultPlan: 'monthly' | 'lifetime' =
    initialPlan ?? (lifetimePlan.pkg !== null ? 'lifetime' : 'monthly');
  ...
  const canPurchase = !offeringsError;
      ...
      <div className="w-full max-w-sm mx-4" onClick={e => e.stopPropagation()}>
        <div className="bg-gradient-to-br from-slate-900 via-yellow-950/20 to-slate-900 border-2 border-yellow-500/70 rounded-lg shadow-[0_0_60px_rgba(234,179,8,0.35)] overflow-hidden">
          ...
          <div className="px-5 py-4">
            ...
            <div className="flex gap-2 mb-7 relative">
              ...
              {showLoadingPlans && (
                <p className="absolute -bottom-5 left-0 right-0 font-orbitron text-[8px] text-yellow-500/60 uppercase tracking-widest text-center animate-pulse">
                  Loading Google Play prices in background...
                </p>
              )}
            </div>
